...
void ModelManager::createModel(std::string name, const char* filepath) {
	...
	mModels.emplace(name, std::move(model));
}

const Model& ModelManager::getModel(std::string name) const {
...
void ModelManager::processModel(aiNode* node, aiMesh** meshes, Model& model) {
    ...
    model.mTransformOffset = transformOffset;
    ...
    for (unsigned int i = 0; i < node->mNumMeshes; i++) {
        ...
        for (unsigned int f = 0; f < mesh->mNumFaces; f++) {
            ...
            for (unsigned int j = 0; j < face.mNumIndices; j++) {
                uint16_t index = face.mIndices[j] + vertexOffset;
        ...
        model.mVertexCount += vertexCount;
        model.mIndexCount += indexCount;
